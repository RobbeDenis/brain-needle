...
pub fn compare_generate_ir_all(c: &mut Criterion) 
{
    let mut group = c.benchmark_group("Generate IR: mandelbrot");
...
pub fn compare_generate_ir_max(c: &mut Criterion)
{
    let file_path = std::path::PathBuf::from("src-bf\\mandelbrot_max.bf");
    let mut group = c.benchmark_group("Generate IR: mandelbrot_max.bf");
    group.sample_size(200);
    group.measurement_time(std::time::Duration::new(20, 0));
    ...
    group.bench_function("Old", |b| {
        b.iter_with_setup(|| {
            ...
            |reader| {
                create_flat_instr_tree_from_tokens(tokenize_file_from_path(reader).unwrap())
            }
        )
    });
...
// criterion_group!(benches, compare_generate_ir_max);
// criterion_group!(benches, compare_generate_ir_all);
